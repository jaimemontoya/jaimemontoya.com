<?php
  ...
  if(isset($_GET["submit"]) && $_GET["reporttype"]=="Income"){
	...
	$sqlSales = " SELECT a.Description, a.Date, a.City, a.[Payment method], a.Buyer, a.Total, (SELECT SUBSTRING((SELECT DISTINCT ', ' + CONVERT(VARCHAR(255),dca.CategoryName) FROM DimCategory dca WHERE dca.CategoryName = a.Category FOR XML PATH, TYPE).value('.[1]','NVARCHAR(MAX)'), 2, 2000)) AS Category, (SELECT SUBSTRING((SELECT DISTINCT ', ' + CONVERT(VARCHAR(255),pca.ParentCategoryName) FROM DimParentCategory pca WHERE pca.ParentCategoryName = a.ParentCategory FOR XML PATH, TYPE).value('.[1]','NVARCHAR(MAX)'), 2, 2000)) AS 'Parent Category' FROM (SELECT Description, FullDateAlternateKey Date, CityName City, PaymentMethodName 'Payment method', BuyerName Buyer, salesTable.CategoryName Category, salesTable.ParentCategoryName ParentCategory, TotalSales Total FROM (SELECT TotalSales, Description, dd.FullDateAlternateKey, dc.CityName, dpm.PaymentMethodName, db.BuyerName, dca.CategoryName, pca.ParentCategoryName FROM FactSales fs INNER JOIN DimCity dc ON fs.CityID=dc.CityID INNER JOIN DimPaymentMethod dpm ON fs.PaymentMethodID=dpm.PaymentMethodID INNER JOIN DimBuyer db ON fs.BuyerID=db.BuyerID INNER JOIN FactSalesXDimCategory fsxdc ON fs.CityID=fsxdc.CityID AND fs.DayID=fsxdc.DayID AND fs.PaymentMethodID=fsxdc.PaymentMethodID AND fs.BuyerID=fsxdc.BuyerID AND fp.CategoryID=fpxdc.CategoryID AND fp.CategoryDeduplicate=fpxdc.CategoryDeduplicate INNER JOIN DimCategory dca ON fsxdc.CategoryID=dca.CategoryID INNER JOIN DimParentCategory pca ON dca.ParentCategoryID=pca.ParentCategoryID INNER JOIN DimDay dd ON fs.DayID=dd.DayID";
    if(isset($_GET['category'])){
