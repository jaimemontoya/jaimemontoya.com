  ...
  if(isset($_GET["submit"]) && $_GET["reporttype"]=="Expenses"){
    ...
    if ($rows === true) {
      ...
      while ($row = sqlsrv_fetch_array($resultExpenses, SQLSRV_FETCH_ASSOC)) {
		...
		$finances->content .=
        "\t\t\t<tr><td>".$row["Description"]."</td><td>".$row["Date"]->format('d/m/Y')."</td><td>".$row["City"]."</td><td>".preg_replace('/[0-9]/','*',$row["Payment method"])."</td><td>".$row["Seller"]."</td><td>".$row["Category"]."</td><td>".$row["Total"]."</td></tr>";
      }
      $finances->content .=
      "\t\t</table>";
	}	
