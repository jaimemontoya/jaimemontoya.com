...
<html>
	...
	<body>		
		...
		*/
		...
		if (validateDate($_GET['startDateKey']) && validateDate($_GET['endDateKey'])) {
			//$sqlExpenses .= " WHERE Date >= '".$_GET['startDateKey']."' AND Date <= '".$_GET['endDateKey']."' AND Category IN ('".implode(', ', explode(", ", $List))."')";
			$sqlExpenses .= " WHERE Date >= '".$_GET['startDateKey']."' AND Date <= '".$_GET['endDateKey']."' AND Category LIKE ('".str_replace(",", "','", $List)."')";
		}
		echo $sqlExpenses;
